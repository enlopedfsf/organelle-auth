...
- **PLANT_SR_ASSEMBLY** — uses nf-core `getorganelle/fromreads` + `getorganelle/config` (apply-time premise change: nf-core now ships a GetOrganelle module, so per TEST-007 we reuse it instead of self-building; see design Decision 4). It runs methods Scenario A's two algorithm-parameter sets exactly: `embplant_pt` (`-R 15 -k 21,45,65,85,105`) and `embplant_nr` (`-k 35,85,115`), injected via `ext.args`. `-R`/`-k` are **GetOrganelle algorithm parameters** (§9.3 → modules.config + design, same in all profiles), NOT scientific acceptance thresholds. The scientific `-F`/`-R`/`-k` stay verbatim; only the reference-delivery mechanism changes (pinned `--config-dir` db vs runtime auto-download — stronger reproducibility, not a scientific change). A small **local output-adapter module** restructures nf-core output to the frozen ①→② interface-contract paths and detects circularization. The module MUST emit `versions.yml`. No forced circularization: low-coverage → scaffold + `DRAFT`.
