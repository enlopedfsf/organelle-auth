...
4. **Behavioral regression was specified but not completed in this change.** CI passed the merged PRs, but no archived artifact proves a baseline-versus-after full nf-test result-set comparison. That residual verification is explicitly superseded to a follow-up change.
...
`PARTIAL/SUPERSEDED`. PRs #13 and #14 merged the metadata repair and restored a green CI lint check. The remaining TODO cleanup, version-warning audit, and full before/after nf-test comparison were not completed and must not be represented as completed tasks here.
