...
include { PLANT_LONG_READ_PILOT } from '../subworkflows/local/plant_long_read_pilot'
...
workflow ORGANELLEAUTH {
    ...
    lr_pilot = PLANT_LONG_READ_PILOT(
        ch_plant_lr,
        file(params.long_read_pilot_reference_fasta),
        file(params.long_read_pilot_manifest)
    )
    ...
    log.info "[organelleauth] M1/M2 short-read routes remain unchanged. M3 plant long-read pilot is isolated EXPERIMENTAL evidence and never feeds IDENTIFY/DECISION."
