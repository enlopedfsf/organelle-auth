process FLYE_SUBSET {
    ...
    def flyeArgs = policyComplete ? "${readFlag} ${reads} --genome-size ${flye.genome_size}${assemblyCoverageArg} --iterations ${flye.iterations} --min-overlap ${flye.min_overlap}" : ''
    """
    set -euo pipefail
    FLYE_POLICY_REVISION=5
    INPUT_SHA256=\$(sha256sum ${reads} | cut -d' ' -f1)
    FLYE_VERSION=\$(flye --version 2>&1 | head -n 1 || true)
    [ -n "\$FLYE_VERSION" ] || FLYE_VERSION=UNAVAILABLE
    printf '%s\n%s\n' 'flye ${flyeArgs} --threads ${task.cpus} --out-dir ${meta.id}.${assembly_label}.flye' "version=\$FLYE_VERSION" > ${meta.id}.${assembly_label}.flye-command.txt
